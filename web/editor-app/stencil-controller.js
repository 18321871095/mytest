...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
        ...
        var modelerStatus=href.split("&")[1].split("=")[1];
        var myregister="";
        var mytime="";
        if(href.split("&").length>3){
            myregister =href.split("&")[3].split("=")[1];
            mytime  =href.split("&")[4].split("=")[1];
        }else{
            myregister="true";
            mytime="";
        }

        $scope.myregister=myregister=='true'?"false":"true";
        $scope.mytime=mytime;

        $scope.modelerStatus=modelerStatus;
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            $http({method: 'GET', url: KISBPM.URL.getStencilSet()}).success(function (data, status, headers, config) {
            	var quickMenuDefinition = ['UserTask', 'EndNoneEvent', 'ExclusiveGateway', 'ParallelGateway','InclusiveGateway',
                ...
                availableQuickMenuItems.splice(6,1);

                $scope.quickMenuItems = availableQuickMenuItems;
               // console.log(availableQuickMenuItems)
        ...
        $scope.propertyClicked = function (index) {
           ...
           // $scope.selectedItem.properties[1].value.assignment.assignee*/
            if (!$scope.selectedItem.properties[index].hidden) {
               // console.log($scope.selectedItem.properties)
                if ($scope.selectedItem.properties[index].key != 'oryx-process_id') {
                    $scope.selectedItem.properties[index].mode = "write";
                }
                if ($scope.selectedItem.properties[index].key == "oryx-seccondautobypass") {
                 //   console.log($scope.selectedItem.properties)
                    var flag=false;
                    if( $scope.selectedItem.properties[1].value!=''){
                        if($scope.selectedItem.properties[1].value.assignment.candidateUsers[0].value!=''){
                            flag=true;
                        }
                    }
                   if($scope.selectedItem.properties[3].value=='Parallel' || $scope.selectedItem.properties[3].value=='Sequential' ||
                       $scope.selectedItem.properties[5].value!='' || flag){
                       $scope.selectedItem.properties[index].value = "false";
                       $scope.selectedShape.properties["oryx-seccondautobypass"] = "false";
                       alert("第一次通过,第二次默认通过只适用于添加办理人栏位中的单任务，不适用于抢占式任务与会签任务，" +
                           "节点类型只能选择None，不能添加会签人")
                   }
                }
                if( $scope.selectedItem.properties[index].key == "oryx-autobypass"){
                    var flag1=false;
                    if( $scope.selectedItem.properties[1].value!=''){
                        if($scope.selectedItem.properties[1].value.assignment.candidateUsers[0].value!=''){
                            flag1=true;
                    ...
                    if($scope.selectedItem.properties[3].value=='Parallel' || $scope.selectedItem.properties[3].value=='Sequential' ||
                        $scope.selectedItem.properties[5].value!='' || flag1){
                        $scope.selectedItem.properties[index].value = "false";
                        $scope.selectedShape.properties["oryx-autobypass"] = "false";
                        alert("自动流转只适用于添加办理人栏位中的单任务，不适用于抢占式任务与会签任务，" +
                            "节点类型只能选择None，不能添加会签人")
                    }
                }
                if($scope.selectedItem.properties[index].key == "oryx-zhuanban"){
                  //  console.log($scope)
                   // console.log( $scope.selectedItem.properties[1].value)
                   // console.log($scope.selectedItem.properties[1].value["assignment"].candidateUsers[0].value)
                    if($scope.selectedItem.properties[1].value==''){
                        if($scope.selectedItem.properties[3].value=='Parallel' || $scope.selectedItem.properties[3].value=='Sequential' ||
                            $scope.selectedItem.properties[5].value!=''){
                            $scope.selectedItem.properties[index].value = "false";
                            $scope.selectedShape.properties["oryx-zhuanban"] = "false";
                            alert("转办只适用于添加办理人栏位中的单任务，不能添加候选人，节点类型只能选择None，不能添加会签人")
                        }
                    }else{
                        if($scope.selectedItem.properties[3].value=='Parallel' || $scope.selectedItem.properties[3].value=='Sequential' ||
                            $scope.selectedItem.properties[5].value!='' ||
                            $scope.selectedItem.properties[1].value["assignment"].candidateUsers[0].value!=''){
                            $scope.selectedItem.properties[index].value = "false";
                            $scope.selectedShape.properties["oryx-zhuanban"] = "false";
                            alert("转办只适用于添加办理人栏位中的单任务，不能添加候选人，节点类型不能只能选择None，不能添加会签人")
                        }
                    }

                }
                if($scope.selectedItem.properties[index].key == "oryx-issetaddhuiqian"){
                    if($scope.selectedItem.properties[3].value!='Parallel'){
                        $scope.selectedItem.properties[index].value = "false";
                        $scope.selectedShape.properties["oryx-issetaddhuiqian"] = "false";
                        alert("加签只适用于会签任务，节点类型只能选择Parallel(并行会签)")
                    }
                }
          ...
          // console.log($scope)
           ...
           // console.log($scope.selectedShape)
            ...
            if(property.key=='oryx-usertaskassignment'){
               ...
               // console.log($scope.selectedItem)
                if($scope.selectedItem.properties[1].value!=''){
                    $scope.selectedItem.properties[3].value='None';
                    $scope.selectedItem.properties[5].value='';
                    $scope.selectedShape.properties["oryx-multiinstance_type"]='None';
                    $scope.selectedShape.properties["oryx-huiqian"]='';
                    //加签
                    $scope.selectedItem.properties[11].value='false';
                    $scope.selectedShape.properties["oryx-issetaddhuiqian"]='false';
                    //抢占式任务
                    if($scope.selectedItem.properties[1].value["assignment"].candidateUsers[0].value!=''){
                        //转办
                        $scope.selectedItem.properties[8].value='false';
                        $scope.selectedShape.properties["oryx-zhuanban"]='false';
                        //第一次通过。。。与自动流转
                        $scope.selectedItem.properties[13].value='false';
                        $scope.selectedShape.properties["oryx-seccondautobypass"]='false';
                        $scope.selectedItem.properties[14].value='false';
                        $scope.selectedShape.properties["oryx-autobypass"]='false';
                    }
                }
            }else if(property.key=='oryx-multiinstance_type' || property.key=='oryx-huiqian'){
                //触发源是会签
                //会签，注意第二次回显数据时，办理人那里是${list}，这种情况需要排除
                if($scope.selectedItem.properties[3].value!='None' || $scope.selectedItem.properties[5].value!=''){
                    if($scope.selectedItem.properties[1].value!=''){
                        var assign = $scope.selectedItem.properties[1].value.assignment.assignee;
                     /*   if(assign.indexOf("$")==-1){*/
                            $scope.selectedItem.properties[1].value='';
                            $scope.selectedShape.properties["oryx-usertaskassignment"]='';
                        //}
                    }
                    //自动流转 第二次默认通过
                    $scope.selectedItem.properties[13].value='false';
                ...
                if(property.key=='oryx-multiinstance_type'){
                    if($scope.selectedItem.properties[3].value=='None'){
                        $scope.selectedItem.properties[5].value='';
                        $scope.selectedShape.properties["oryx-huiqian"]='';
                    }
                }
                if( property.key=='oryx-huiqian'){
                    $scope.selectedItem.properties[3].value='Parallel';
                    $scope.selectedShape.properties["oryx-multiinstance_type"]='Parallel';
                }
